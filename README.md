...
- 保存最近 20 次 Session 的本地观测记录，刷新页面后可以继续查看。
...
```text
local-agent-demo/.data/session-history.json
```

刷新页面或重启 Demo 后，可以在“最近运行”中恢复这些记录。API Key 不会进入该文件；服务重启后仍需重新填写 API Key，才能发起新的请求。
...
- Session 历史可能包含用户输入、Agent 回复和产物元数据；如涉及敏感内容，应及时清理 `.data/`。
...
普通页面刷新不会清除 API Key；停止并重新启动本地服务后会清除。这是 Demo 的安全设计，避免把密钥持久化到磁盘。
